...
// double bottomwall_func(double x, double h1, double h2, double l) {
  ...
  //   return ((h1 - h2) / 2.0) * (1.0 - cos(M_PI * (x / l)));
// }
...
double bottomwall_func(double x, double h1, double h2, double l) {
	double s=x/l;
	return (h1-h2)*s*s*s*(s*(6.0*s-15.0)+10.0);
}
...
// export functions use const char* to ensure that the filename string is not changed within function. Depending on what input u give, the filename can live in all three memory locations. 
// const does not define where the variable is stored, it makes it read only
